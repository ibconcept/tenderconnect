...
import { AiFillStar, AiOutlineStar } from 'react-icons/ai'; // For star ratings
        ...
        const updatedTenders = tenders.map(tender => {
            if (tender.id === tenderId) {
                return {
                    ...tender,
                    liked: !tender.liked,
                    likes: tender.liked ? tender.likes - 1 : tender.likes + 1
                };
            }
            return tender;
        });
            ...
            {!isLoggedIn ? (
                ...
                <div id="tender-detail">
                    ...
                    <button onClick={() => toggleLike(selectedTender.id)}>
                        <FaThumbsUp /> {selectedTender.likes}
                    </button>
                    <h3>Rating</h3>
                    {[1, 2, 3, 4, 5].map(star => (
                        <span key={star} onClick={() => handleStarRating(selectedTender.id, star)} style={{ cursor: 'pointer' }}>
                            {star <= selectedTender.rating ? <AiFillStar /> : <AiOutlineStar />}
                        </span>
                    ))}
                    <h3>Comments</h3>
                    ...
                    <ul>
                        {selectedTender.comments.map((comment, index) => <li key={index}>{comment}</li>)}
                    </ul>
