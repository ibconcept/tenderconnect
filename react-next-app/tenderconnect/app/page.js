...
const Home = () => {
    ...
    const toggleLike = (tenderId) => {
        const updatedTenders = tenders.map(tender => 
            tender.id === tenderId ? { ...tender, likes: tender.likes + 1 } : tender
        );
        setTenders(updatedTenders);
        saveTendersToLocalStorage(updatedTenders);
    };
        ...
        <div className="container">
            ...
            {!isLoggedIn ? (
                ...
                <div id="tender-detail">
                    ...
                    <button onClick={() => toggleLike(selectedTender.id)}>Like {selectedTender.likes}</button>
                    <h3>Rating</h3>
                    {[1, 2, 3, 4, 5].map(star => (
                        <span key={star} onClick={() => handleStarRating(selectedTender.id, star)} style={{ cursor: 'pointer' }}>
                            {star <= selectedTender.rating ? '★' : '☆'}
                        </span>
                    ))}
                    <h3>Comments</h3>
